...
Date: 2026-08-17 (sizing); 2026-08-24 (side arrows; infinite loop removed)  
...
Desktop: left/right side arrows at the absolute left/right edges of the section (in the pad gutters), plus the bottom pill nav. Mobile: bottom pill only.
...
6. **Desktop side arrows (≥900px):** `.bf-about__side-btn` are section children (`position: absolute; left/right: 0.5rem` on `.bf-section.bf-about`). JS vertically centers them on the card track. Bottom pill stays visible on all breakpoints.
7. Vertical layout: no side arrows (unchanged).
