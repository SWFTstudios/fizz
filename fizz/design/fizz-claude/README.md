...
Ambient rising bubbles appear in **hero**, **how it works**, **footer**, and **PDP stage**. They are rendered with **HTML5 canvas** + `requestAnimationFrame`. Research notes: [`BUBBLE-TRANSITION-RESEARCH.md`](BUBBLE-TRANSITION-RESEARCH.md).
...
The `<html>` element receives `bubble-style-glass` (or `ring` / `foam` / `bold`), optional `bubble-wobble`, `data-bubble-enabled`, and transition data attrs when the `fizz-claude` template is active.

### Page transitions

Internal navigation overlay in `assets/fizz-claude-transition.js`:
...
Colors pull from `--hero-accent` / `--hero-bg` / `--hero-text`. Intensity and duration are merchant-scalable under **Fizz Claude bubble animation**. Links with `data-no-transition` skip the effect. `prefers-reduced-motion` navigates instantly.
