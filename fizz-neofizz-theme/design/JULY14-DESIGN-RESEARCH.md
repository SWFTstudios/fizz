...
Consequence adopted in this theme: every media block (`nf-intro`,
`nf-mosaic`, `nf-how-sticky`, `nf-flavors`, `nf-about`) exposes all three
inputs plus a theme-asset fallback text field, with priority
video → external video → image → asset → placeholder
(implemented in `snippets/nf-media.liquid`).
...
- `enabled_on` / `disabled_on` restrict where sections can be added; used so
  e.g. `nf-intro` only appears on the home template and `nf-product` only
...
- Storefront colors resolve from `settings.colorway_preset` via
  `snippets/nf-colorway-preset-data.liquid` → `nf-theme-tokens.liquid`.
...
- **Liquid cannot parse JSON files at runtime**, so bottle/scene colorway
  ...
  `snippets/nf-colorway-scene.liquid`, with per-variant metafield
...
- Scroll-driven effects (intro expansion, mosaic slide-up, sticky how-to)
  are storefront runtime JS (`assets/nf-scroll.js`). The theme editor does
...
- Colorway-aware PDP stage gradients reuse
  `snippets/nf-colorway-scene.liquid` (variant/bottle palette). Global
  Theme style / colorway presets still flow through
  `snippets/nf-theme-tokens.liquid` (`--nf-paper`, `--nf-ink`,
  `--nf-accent`, `--nf-surface`, `--nf-dark`, fonts, radii) so switching
...
- Colorway slug/scene logic was adapted (copied and renamed with the `nf-`
...
2. Paper fill layer using `background: var(--nf-paper)`.
...
`assets/nf-intro.js` and matching CSS origins. Replacing a mask asset
...
`sections/nf-footer.liquid` as a global footer-group section:
...
- Content priority for background: Shopify `video` → `image` → theme `asset` filename → placeholder (via `snippets/nf-media.liquid`). **No `video_url`** for this layer.

## 8. Glassmorphic stats section (reusable page section)

`sections/nf-stats.liquid` reuses the same cover-media + frosted-glass pattern as
...
- Count-up numbers are **client-only** (`assets/nf-stats.js` + IntersectionObserver).
...
| --- | --- | --- |
| CSS mask RGB ≠ brand paper | Asset export | Paper from `--nf-paper`; alpha mask only |
