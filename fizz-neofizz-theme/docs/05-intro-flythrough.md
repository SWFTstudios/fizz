...
| File | Role |
| --- | --- |
| `sections/nf-intro.liquid` | Markup, CSS vars, schema, slides |
| `assets/nf-intro.js` | GSAP timeline + auto-scroll |
| `assets/nf-base.css` | Mask / stage / copy styles |
...
```liquid
--nf-intro-mask: url('{{ 'Fizz_Logo_Intro.svg' | asset_url }}');
--nf-intro-mask-mobile: url('{{ 'Fizz_Logo_INTRO_SVG_Mobile.svg' | asset_url }}');
```
...
`transform-origin` and `getMaskMetrics()` in `nf-intro.js`.
...
ScrollTrigger on `.nf-intro__track` (`height: scroll_length vh`):
...
4. Validate in `preview/intro-mask-flythrough.html` (press **D** for origin
   debug in the lab).
5. Push `nf-intro.liquid`, `nf-base.css`, `nf-intro.js`, and the SVG(s).
