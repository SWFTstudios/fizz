...
**Fix:** Re-measure stem; update CSS + `getMaskMetrics()` in `nf-intro.js`.
...
**Fix:** Ensure `nf-intro.js` is loaded; cancel events are on `window`.
...
Use **melt** style (not classic) and current `nf-page-transition.js`. Classic
