...
`settings.colorway_preset` → `nf-colorway-preset-data` → CSS variables in
`nf-theme-tokens`. Custom pickers override when
...
Without metafields, `nf-color-slug` derives a handle from the variant title
and `nf-colorway-scene` returns a baked gradient. That is intentional —
...
- Metafield values are read at Liquid render time into `data-*` / inline
  styles.  
- Active-slide scene sync is client-side in `nf-carousel.js`.  
- Changing Theme style re-renders tokens on save; carousel slides keep their
  per-variant scene data.

## Known gap

Section setting `scroll_height_per_colorway` and `data-nf-colorway-scroll` are
