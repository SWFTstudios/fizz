...
├── sections/        # nf-* merchant sections + header-group.json + footer-group.json
...
- `nf-scroll.js` handles mosaic / how-to without GSAP.
...
1. `nf-base.css`
2. `nf-theme-tokens` snippet (CSS custom properties)
3. Header group sections
4. `{{ content_for_layout }}`
5. Footer group sections (`footer-group.json` → `nf-footer`)
6. Deferred scripts: GSAP → ScrollTrigger → intro → scroll → carousel →
   page transitions

`class="nf-no-motion"` is applied when Theme settings disable motion or when
...
| Prefix | Meaning |
| --- | --- |
| `nf-` | July 14 theme component |
| `data-nf-*` | JS hooks |
| `--nf-*` | CSS custom properties |
